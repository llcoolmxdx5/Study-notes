...
1.urlopen(url,data=None,[timeout])
  ...
  返回:response对象
  res.read().decode()
2.Request(url,data=None,headers={},origin_req_host=None,
          unverifiable=False,method='')
  data:bytes类型
...
5.urljoin(base_url,new_url)
  会分析base_url的scheme、netloc和path这3个内容
  并对新链接缺失的部分进行补充，最后返回结果。
6.urlencode(dict)
  将字典序列化为GET请求参数
7.parse_qs()
...
2.HTTPError
  ...
  属性:code：返回HTTP状态码 
      ...
      headers：返回请求头
3.URLError是HTTPError的父类，所以可以先选择捕获子类的错误，
  再去捕获父类的错误
